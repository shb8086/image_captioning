...
In order to determine the length of a caption, we conducted a thorough analysis of all prepositions and postpositions, such as\begin{farsi}
\arabicfont\small به\end{farsi} (to),\begin{farsi}
  \arabicfont\small و \end{farsi}(and),\begin{farsi}
    \arabicfont\small می \end{farsi}(A non-combining prefix is employed in both the present and imperfect tenses), and others. Additionally, we discovered that the shortest caption length observed was for image 1370615506, consisting of only two words: "نوزادی میخندد" (an infant is smiling) (refer to Fig. 1 and 2 for visual representation).
